...
make -w -C sql GPRBUILD_OPTIONS="--db $script_loc/config"
make -w -C sql install

make -w -C sqlite setup prefix=$PREFIX
make -w -C sqlite GPRBUILD_OPTIONS="--db $script_loc/config"
...
make -w -C gnatinspect GPRBUILD_OPTIONS="--db $script_loc/config"
make -w -C gnatinspect install

make -w -C gnatcoll_db2ada setup prefix=$PREFIX DB_BACKEND=sqlite
make -w -C gnatcoll_db2ada GPRBUILD_OPTIONS="--db $script_loc/config"
